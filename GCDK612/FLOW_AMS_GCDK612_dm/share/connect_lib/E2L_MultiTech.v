...
//============================================================================
  initial begin          // interface nodes always start either HI or LO:
    ...
    #0.1 Dreg = (V(Ain)>(V(\VDDA! )/1.5))? 1'b1 :            // wait till analog start 
                 (V(Ain)<(V(\VDDA! )/3.0))? 1'b0 : 1'bx;     //  and compute actual level.
  end
  always @(cross(V(Ain)-(V(\VDDA! )/1.5),0,ttol,(V(\VDDA! )/10.0))) begin // upper threshold crossing
    if (V(Ain)>(V(\VDDA! )/1.5)) begin
      Dreg=1; Xin=0;                             // output goes high
    end
    else Xin=1;                                  // output in between
  end
  always @(cross(V(Ain)-(V(\VDDA! )/3.0),0,ttol,(V(\VDDA! )/10.0))) begin // lower threshold crossing
    if (V(Ain)<(V(\VDDA! )/3.0)) begin
