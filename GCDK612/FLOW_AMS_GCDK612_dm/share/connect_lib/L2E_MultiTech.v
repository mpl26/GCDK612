...
//============================================================================
  initial begin
    ...
    case (Din)     // Convert initial Din to equivalent voltage & log of Rout:
      ...
      1'bz:    begin  Vout=(V(\VDDA! )/2.0);  Rlog=ln(rz);   end     // init Z
      default: begin  Vout=(V(\VDDA! )/2.0);  Rlog=ln(rx);   end     // init X
    endcase
  ...
  always  @Din begin  
    case (Din)                        // Lookup of V,R,T values for new state
      ...
      1'bz:  begin                    // to HIGH IMPEDANCE
         Vout=(V(\VDDA! )/2.0);  Rlog=ln(rz);  TdV=tz/2;
         TrV=tz/2; TrR=tz;                               // Hi/Lo/X to Z
      end
      default:  begin                 // to UNKNOWN
         Vout=(V(\VDDA! )/2.0);  Rlog=ln(rx);  TdV=0;
