...
//============================================================================
  initial begin
    ...
    case (Din)     // Convert initial Din to equivalent voltage & log of Rout:
      ...
      1'bz:    begin  Vout=(V(\VDD! )/0.58333);  Rlog=ln(rz);   end     // init Z
      default: begin  Vout=(V(\VDD! )/0.58333);  Rlog=ln(rx);   end     // init X
    endcase
  ...
  always  @Din begin  
    case (Din)                        // Lookup of V,R,T values for new state
      ...
      1'bz:  begin                    // to HIGH IMPEDANCE
         Vout=(V(\VDD! )/0.58333);  Rlog=ln(rz);  TdV=tz/2;
         TrV=tz/2; TrR=tz;                               // Hi/Lo/X to Z
      end
      default:  begin                 // to UNKNOWN
         Vout=(V(\VDD! )/0.58333);  Rlog=ln(rx);  TdV=0;
  ...
  always  @(cross(V(Aout)-(V(\VDD! )/2.0),0,ttol,(V(\VDD! )/10.0))) begin  // lower threshold crossed
    if (V(Aout)<(V(\VDD! )/2.0))  begin
