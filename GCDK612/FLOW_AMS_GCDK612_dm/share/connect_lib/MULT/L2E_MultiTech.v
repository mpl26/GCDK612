...
//============================================================================
connectmodule L2E_MultiTech (Din, Aout);
  input Din; logic Din;                       // input logic
  output Aout; electrical Aout;               // output electrical
...
//============================================================================
  initial begin
    ...
    case (Din)     // Convert initial Din to equivalent voltage & log of Rout:
      1'b1:    begin  Vout=V(\VDD! ); Rlog=ln(rhi);  end     // init Hi
      1'b0:    begin  Vout=vlo; Rlog=ln(rlo);  end     // init Lo
      1'bz:    begin  Vout=(V(\VDD! )/2.0);  Rlog=ln(rz);   end     // init Z
      default: begin  Vout=(V(\VDD! )/2.0);  Rlog=ln(rx);   end     // init X
    endcase
    TdV=0; TrV=tr; TrR=tr;            // initially default delay&rise terms
    Dold=Din;                         // keep old value of Din
    #0.1 Dreg=(V(Aout)>(V(\VDD! )/1.5))? 1'b1 :  // Drive digital from analog DC level
               (V(Aout)<(V(\VDD! )/3.0))? 1'b0 : 1'bx;
  end

  always  @Din begin  
    case (Din)                        // Lookup of V,R,T values for new state
      1'b1:  begin                    // to HIGH
         Vout=V(\VDD! );  Rlog=ln(rhi);  TdV=0;
      ...
      1'bz:  begin                    // to HIGH IMPEDANCE
         Vout=(V(\VDD! )/2.0);  Rlog=ln(rz);  TdV=tz/2;
         TrV=tz/2; TrR=tz;                               // Hi/Lo/X to Z
      end
      default:  begin                 // to UNKNOWN
         Vout=(V(\VDD! )/2.0);  Rlog=ln(rx);  TdV=0;
         TrV=tx;  TrR=tx;                                // Hi/Lo/Z to X
         Dreg=1'bx;                   // logic X occurs immediately
      end
    endcase
    Dold=Din;                         // keep old value of Din
     // ADDED BY ROB M. TO JUST FORCE DREG TO TAKE DIGITAL VALUE
     Dreg=Din;
  end
  ...
  analog begin                 // Apply Vout,Rout to output with transitions:
   ...
   // COMMENTED OUT BY ROB M. SO ANALOG DOES NOT DRIVE DIGITAL
   // Reflect actual levels on analog side back to digital side:
   // always  @(cross(V(Aout)-(V(\VDD! )/1.5),0,ttol,(V(\VDD! )/10.0))) begin  // upper threshold crossed
   //  if (V(Aout)>(V(\VDD! )/1.5))  begin
   ...
   // end
   // always  @(cross(V(Aout)-(V(\VDD! )/3.0),0,ttol,(V(\VDD! )/10.0))) begin  // lower threshold crossed
   //   if (V(Aout)<(V(\VDD! )/3.0))  begin
   ...
   //  NEXT LINES COMMENTED OUT BY ROB MEHREGAN 01/09/03
   //  BECAUSE IT CAUSES UNKNOWNS BACK ON THE DIGITAL SIDE  
   //   
